...
from utils import extract_fits_header, unicoord


def flux_extractor_to_dict(path, coords, stokes, radius, rms_radius, scale, units="mJy"):
    """
    Extract flux density and related information from a FITS image using CASA's imfit and imstat tasks.
    ____
    path : str
        Path to the FITS image file.
    coords : astropy.coordinates.SkyCoord
        Coordinates of the source to fit.
    stokes : str
        Stokes parameter to use for the fit (e.g., 'I').
    radius : float
        Radius in arcseconds for the fitting region.
    rms_radius : float
        Radius in arcseconds for the RMS estimation region.
    scale : float
        Scale factor for the flux density error estimation.
    units : str, optional
        Units for flux density ('mJy' or 'uJy'). Default is 'mJy'.
    """
    ...
    region = f"circle[[{region_temp}], {radius}arcsec]"
    annulus_region = f"annulus[[{region_temp}], [{2*radius}arcsec, {rms_radius}arcsec]]"

    source_name = "J" + region_temp[0:2] + region_temp[3:5]
    name = f"{source_name}_{stokes}_{radius}_arcsec"
    summary = f"fit_summary_{name}"
    logfile = f"fit_log_{name}"
...
#     "-u",
#     "--units",
#     type=click.Choice(["mJy", "uJy"]),
#     default="mJy",
#     show_default=True,
#     help="Flux density units",
# )
...
@click.argument("rms_radius", nargs=1, type=float, default=30)
@click.argument("scale", nargs=1, type=float, default=0.1)
def batch(paths, coords, stokes, radius, rms_radius, scale, galactic, output, units):  # noqa: D103
    pos_eq, __ = unicoord(coords, galactic, display=False)
    rows = []
    for path in paths:
